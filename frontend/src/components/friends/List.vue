<template>
	<PageHeader heading="Your friends">
		<template #actions>
			<Button variant="outline" @click="showInviteFriendsDialog = true"
	...
	<div v-else>You have not invited your friends yet.</div>

	<div
		class="mt-4"
		v-if="invitationListResource.list.data && invitationListResource.list.data.length > 0"
	>
		...
		<ul class="space-y-2 mt-4">
			<li class="flex justify-between" v-for="invite in invitationListResource.list.data">
				...
				<Badge :theme="invite.status == 'Not Sent' ? 'red' : 'orange'">{{
					invite.status
				}}</Badge>
			</li>
		</ul>
	...
	<Dialog :options="{ title: 'Invite Friend' }" v-model="showInviteFriendsDialog">
		<template #body-content>
			<div class="space-y-3">
				<FormControl
					v-model="inviteDetails.invitee_name"
					type="text"
					label="Name"
					placeholder="Jane Doe"
				/>
				<FormControl
					v-model="inviteDetails.invitee_phone"
					type="tel"
					label="Mobile Number"
				/>
			</div>
